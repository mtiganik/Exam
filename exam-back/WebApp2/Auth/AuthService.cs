...
namespace WebApp2.Auth
{
    ...
    IOptions<JwtAuthConfig> jwtAuthConfig, ILogger<AuthService> logger, SignInManager<AppUser> signInManager)
        ...
        public async Task<JWTResponse> Register(RegisterInfo model)
        {
            var appUser = await _userManager.FindByEmailAsync(model.Email);
            if (appUser != null)
            {
            ...
            (appUser, var refreshToken) = GenerateNewUserData(model.Email, model.UserName);
        ...
        private (AppUser, AppRefreshToken) GenerateNewUserData(string email, string userName)
        {
            var refreshToken = new AppRefreshToken();
